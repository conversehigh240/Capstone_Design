...
def draw_keypoints(frame, keypoints, confidence_threshold):
    ...
    new_keypoints = np.multiply(keypoints, [height, width, 1])
    shaped = np.squeeze(np.multiply(keypoints, [height, width, 1]))

    youtube_keypoints.append(new_keypoints)
...
def save_youtube(id):
    ...
    # subprocess.run(['ffmpeg', '-ss', '15', '-i', 'static/input.mp4', '-c', 'copy', 'static/input_2.mp4'])
